...
import { CODE_BLOCK_LANGUAGE_SYNTAX_DEFAULT } from '../../constants/code-languages';
    ...
    view(view) {
      class LowlightPluginView {
        ...
        async initDecorations() {
          ...
          const languages = [
            ...codeBlocks.map(
              (block) => block.node.attrs.language || CODE_BLOCK_LANGUAGE_SYNTAX_DEFAULT,
            ),
            defaultLanguage,
          ];
        ...
        async checkUndecoratedBlocks() {
          ...
          const loadStates = await Promise.all(
            codeBlocks.flatMap((block) => [
              loadLanguage(
                block.node.attrs.language || CODE_BLOCK_LANGUAGE_SYNTAX_DEFAULT,
                lowlight,
              ),
            ]),
          );
